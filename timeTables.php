...
$con = mysqli_connect("localhost","root","","icareunew") or die("Can't Connect to the Database.");
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($rows == 0) {
        for ($i = 0; $i < 7; $i++) {
            ...
            $data       = $data . '<td><center>' . "Available " . "<br>" . "Appointment No: " . $appoNumber . '</center></td>';
    ...
    if ($rows != 0) {
        for ($i = 0; $i < 7; $i++) {
            ...
            $appoNumber = patientnumber($appdate, $id);
            $data       = $data . '<td><center>' . "Appointment No: " . $appoNumber . '</center></td>';
...
<div style="text-align: left;font-size: 20px;font-family: Arial,Helvetica,sans-serif;margin-left: 20px;font-weight: 500;color: rgb(48, 71, 112);">
   This is our schedule Today.
</div><br><br><?php //(Person='general' OR PhysicianID=$id) AND
$query  = "SELECT Person, Date, `From`, `To` FROM calender WHERE Person='general' AND Date = '" . date('Y-m-d') . "';";
$result = mysqli_query($con, $query);
$rows = mysqli_num_rows($result);
if ($rows == 0) {
    
    echo "<div style='text-align: left;font-size: 17px;font-family: Arial,Helvetica,sans-serif;margin-left: 40px;font-weight: 500;color: rgb(0,0,0);'>Available Physicians: All</div>" . "<br>";
}
else{
    $record = mysqli_fetch_assoc($result);
    echo "<div style='text-align: left;font-size: 17px;font-family: Arial,Helvetica,sans-serif;margin-left: 40px;font-weight: 500;color: rgb(0,0,0);'>We are closed today ";
    echo "from ".$record['From'] . ' To ' . $record['To'].'</div>';
}
?>
<br>
...
<div id="footerBottom" style="position:fixed;bottom:6;left:6;right:6;">Copyright © 2017 all rights reserved</div>
</body>
</html>
